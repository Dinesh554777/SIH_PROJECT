...
import { Search, ChevronLeft, ChevronRight, BrainCircuit, Loader, CheckCircle } from 'lucide-react';
...
const ResourcesPage: React.FC = () => {
    const [resources, setResources] = useState<Resource[]>(RESOURCES_DATA);
    const [indexingState, setIndexingState] = useState<Record<number, 'idle' | 'loading' | 'indexed'>>({});
    ...
    const handleIndexResource = async (resource: Resource) => {
        if (indexingState[resource.id] === 'loading' || indexingState[resource.id] === 'indexed' || resource.isIndexed) return;
        setIndexingState(prev => ({ ...prev, [resource.id]: 'loading' }));
        try {
            await indexDocument(resource);
            setResources(prev => prev.map(r => r.id === resource.id ? { ...r, isIndexed: true } : r));
            setIndexingState(prev => ({ ...prev, [resource.id]: 'indexed' }));
        } catch (error) {
            console.error("Failed to index document:", error);
            setIndexingState(prev => ({ ...prev, [resource.id]: 'idle' }));
        }
    };
    ...
    return (
        <div className="space-y-16">
            <div className="text-center">
                ...
                <p className="text-lg text-text-secondary">Find and index study materials to make them available to your AI Tutor.</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 min-h-[400px]">
                {paginatedResources.map(resource => {
                    const status = indexingState[resource.id] || (resource.isIndexed ? 'indexed' : 'idle');
                    return (
                        <Card key={resource.id} className="flex flex-col">
                            ...
                            <button 
                                ...
                                className={`mt-6 w-full py-2 rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors
                                ${status === 'idle' ? 'bg-primary/80 text-white hover:bg-primary' : ''}
                                ${status === 'loading' ? 'bg-slate-600 text-text-secondary cursor-wait' : ''}
                                ${status === 'indexed' ? 'bg-success/20 text-success cursor-default' : ''}
                            `}>
                                {status === 'idle' && <><BrainCircuit size={18} /> Index for AI</>}
                                {status === 'loading' && <><Loader size={18} className="animate-spin" /> Indexing...</>}
                                {status === 'indexed' && <><CheckCircle size={18} /> Indexed</>}
                            </button>
                        </Card>
                    )
                })}
